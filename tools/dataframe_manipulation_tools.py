...
class DataframeMelt(Tool):
    ...
    def forward(self, frame, id_vars=None, value_vars=None, 
                var_name=None, value_name=None, col_level=None, 
                ignore_index=None):
        import pandas as pd
...
class DataframeConcat(Tool):
    ...
    def forward(self, objs, frame, axis=0, ignore_index=False):
        # Build the concat parameters dict, filtering out None values
        import pandas as pd
        concat_params = {
            "frame": frame,
            "objs": objs,
